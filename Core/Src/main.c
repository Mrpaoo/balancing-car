...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim){
	if (htim->Instance == TIM8 && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
		...
		if (HAL_GPIO_ReadPin(CS100A_ECHO_GPIO_Port, CS100A_ECHO_Pin) == GPIO_PIN_SET) {
			/* Rising edge: ECHO went high, record start time */
			g_sensor_data.csa.echo_start = capture;
		} else {
			/* Falling edge: ECHO went low, compute pulse width */
			if (capture >= g_sensor_data.csa.echo_start) {
				g_sensor_data.csa.echo_width = capture - g_sensor_data.csa.echo_start;
			}
			g_sensor_data.csa.echo_ready = 1;
		}
	}
}
/* USER CODE END 4 */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  if(htim->Instance==SENSOR_TIM_INSTANCE){
	  collect_data(&g_sensor_data);
  }
